...
class ScoringService(object):
    ...
    def get_features_and_label(cls, dfs, codes, feature, label):
        ...
        for code in tqdm(codes):
            ...
            labels_h = stock_labels[f'label_high_{label}'].copy()
            labels_l = stock_labels[f'label_low_{label}'].copy()

            # nanを削除
            labels_h.dropna(inplace=True)
            labels_l.dropna(inplace=True)

            if feats.shape[0] > 0 and labels_h.shape[0] > 0 and labels_l.shape[0] > 0:
                # 特徴量と目的変数のインデックスを合わせる
                labels_h = labels_h.loc[labels_h.index.isin(labels_l.index)]
                feats = feats.loc[feats.index.isin(labels_h.index)]
                labels_h = labels_h.loc[labels_h.index.isin(feats.index)]
                labels_l = labels_l.loc[labels_l.index.isin(feats.index)]
                labels_h.index = feats.index
                labels_l.index = feats.index
                ...
                _train_y = labels_h[: cls.TRAIN_END]
                _val_y = labels_h[cls.VAL_START : cls.VAL_END]
                _test_y = labels_h[cls.TEST_START :]

                _train_yl = labels_l[: cls.TRAIN_END]
                _val_yl = labels_l[cls.VAL_START : cls.VAL_END]
                _test_yl = labels_l[cls.TEST_START :]
                ...
                trains_yl.append(_train_yl)
                vals_yl.append(_val_yl)
                tests_yl.append(_test_yl)
        ...
        train_yl = pd.concat(trains_yl)
        val_yl = pd.concat(vals_yl)
        test_yl = pd.concat(tests_yl)

        # objectives
        train_y = [train_y, train_yl]
        val_y = [val_y, val_yl]
        test_yl = [test_y, test_yl]
...
#             "fundamental+technical": list(fundamental_cols) + list(technical_cols),
    ...
    def fit_mlp(cls, train_X: pd.DataFrame, train_y: np.array, val_X: pd.DataFrame, val_y: np.array):
        ...
        for i in np.arange(params['hidden_layers'] - 1):
            x = tf.keras.layers.Dense(params['hidden_units'] // (params['decay_factor'] * (i+1)), activation=params['hidden_activation'])(x)
            x = tf.keras.layers.BatchNormalization()(x)
            x = tf.keras.layers.GaussianNoise(params['gauss_noise'])(x)
            x = tf.keras.layers.Dropout(params['hidden_dropout'])(x)

        # output
        out_h = tf.keras.layers.Dense(1, activation="linear", name="out_h")(x)
        out_l = tf.keras.layers.Dense(1, activation="linear", name="out_l")(x)
        outs = [out_h, out_l]
        model = tf.keras.models.Model(inputs=inputs, outputs=outs) 

        # compile
        loss = ["mae", "mae"] # mean absolute error as a loss
        loss_weights = [1, 1]
        opt = tf.keras.optimizers.Adam(lr=params['lr'], beta_1=0.9, beta_2=0.999, decay=params['lr']/100)
        model.compile(loss=loss, loss_weights=loss_weights, optimizer=opt, metrics=[])
    ...
    def create_model(cls, dfs, codes, label, model_name='xgb1'):
        ...
        # cv scores
        cvs_h = cls.compute_cv(pred_y[0], val_y[0])
        cvs_l = cls.compute_cv(pred_y[1], val_y[1])

        return model, cvs_h, cvs_l
    ...
    def train_and_save_model(
        ...
        for model_name in model_names:
            for label in labels:
                ...
                # get model
                model, cvs_h, cvs_l = cls.create_model(cls.dfs, 
                    codes=codes, label=label, model_name=model_name)
                
                # assign
                cvs = cvs_h.merge(cvs_l, how='left', on='metric', suffixes=('_high', '_low'))
    ...
    def predict(cls, inputs, labels=None, model_names=None, codes=None, 
        ...
        num_models = 1
        for label in labels:
            df[f'label_high_{label}'] = 0
            df[f'label_low_{label}'] = 0

            for model_name in model_names:
                print(f'{model_name}_{label}')

                # 予測実施                
                preds = cls.models[f'{model_name}_{label}'].predict(feats[feature_columns])
                df[f'label_high_{label}'] += preds[0] / num_models
                df[f'label_low_{label}'] += preds[1] / num_models
